 ...
 * Vincula todos os dados do paciente 'denis-santos' ao userId do owner
 */
export async function migrateOwnerData(ownerUserId: number, ownerEmail: string): Promise<void> {
  ...
  try {
    // 1. Atualizar paciente Denis Santos
    ...
    console.log(`[Migration] Updated patient 'denis-santos' to userId=${ownerUserId}`);
    ...
    console.log(`[Migration] Updated exam_history for patient 'denis-santos' to userId=${ownerUserId}`);
    ...
    console.log(`[Migration] Updated exam_correlations for patient 'denis-santos' to userId=${ownerUserId}`);

    console.log(`[Migration] ✅ Data migration completed successfully!`);
    console.log(`[Migration] All data for 'denis-santos' is now linked to userId=${ownerUserId} (${ownerEmail})`);
  } catch (error) {
